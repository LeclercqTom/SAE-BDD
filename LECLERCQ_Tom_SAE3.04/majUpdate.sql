DROP TRIGGER majUpdate ON "Ventes";
CREATE OR REPLACE FUNCTION majUpdate() RETURNS TRIGGER AS $$
...
BEGIN
	...
	select "Oeuvres"."en_stock" into en_stock from "Oeuvres" where "Oeuvres_id" = NEW."Oeuvres_id";
		if (en_stock is TRUE) THEN
			raise notice '%', NEW."Oeuvres_id";
			raise notice '%', OLD."Oeuvres_id";
			raise notice '%', NEW."Fac_id";
...
--             update "Oeuvres" set "en_stock" = TRUE where "Oeuvres_id" = OLD."Oeuvres_id";
		ELSE
...
CREATE TRIGGER majUpdate
AFTER INSERT OR UPDATE OR DELETE ON "Ventes" FOR EACH ROW EXECUTE FUNCTION majUpdate();